...
router.post('/signup', (req, res) => {
  ...
  if (!username || !password) {
    res.render('auth/signup', { errorMessage: 'ERROR: Rellena los dos campos' })
    return
  }
  ...
  User.findOne({ "username": username })
    .then(user => {
      if (user) {
        res.render("auth/signup", { errorMessage: "El nombre de usuario ya existe" })
        return
      }
...
router.post("/login", (req, res, next) => {
  ...
  if (username === "" || password === "") {
    res.render("auth/login", { errorMessage: "Rellena ambos campos, merluzo." })
    return
  }
